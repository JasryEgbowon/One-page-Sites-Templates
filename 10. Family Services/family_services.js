...
document.querySelector('form')?.addEventListener('submit', function(e) {
    ...
    setTimeout(() => {
        alert('Thank you for your interest! Our partnership team will contact you within 24 hours to discuss how EduSpark can support your school.');
        this.reset();
...
function animateStatsCounter() {
    ...
    stats.forEach((stat, index) => {
        ...
        const timer = setInterval(() => {
            count += increment;
            if (count >= target) {
                stat.textContent = target + (index === 0 ? '+' : (index === 1 ? '+' : ''));
                clearInterval(timer);
            } else {
                stat.textContent = Math.floor(count) + (index === 0 ? '+' : (index === 1 ? '+' : ''));
            }
        }, 16);
    });
}
...
console.log('📚 Welcome to EduSpark - Transforming education one classroom at a time');